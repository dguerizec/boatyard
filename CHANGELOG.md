...
- **v0.17.2: TwiCC in Sync** — Keep the session board and resource monitor compatible with both legacy and newer TwiCC APIs.

### Fixed

- **TwiCC compatibility** — Sessions, metadata, and process status now load correctly across legacy, transitional, and newer TwiCC versions, keeping the board and resource monitor working consistently.
...
- **v0.14.1: Stable Mobile Previews** — Keeps mobile previews at your chosen dimensions while fitting their visible content within pane boundaries.

### Fixed

- **Mobile viewport sizing** — Resizing a pane no longer changes the embedded page’s mobile layout. Your selected dimensions persist across navigation and reloads, and content stays clipped to the pane.
...
- **v0.9.15: GitHub Repository Navigation** — Adds a dedicated GitHub repository experience with sticky sections, an overview pane, and cleaner navigation controls.

### Added

- **GitHub repository pane** — GitHub projects now get a dedicated pane with Overview, Code, Issues, Pull requests, Actions, and Settings sections.
- **GitHub overview** — The new Overview section brings Actions and Pull Requests together in one place.

### Changed

- **Repository tabs** — GitHub repositories now open through the GitHub pane, while the generic Repo pane remains available for other Git hosts.
- **Browser controls** — Repository section panes keep navigation visible and move browser controls into a compact overlay.

### Fixed

- **Workflow visibility sync** — Hiding or restoring GitHub workflow runs now stays consistent across GitHub widgets and the new overview pane.

### Documentation

- **Integration pane docs** — Updated the README and manual to describe provider-specific pane navigation and the new GitHub repository pane.

## [0.9.14] - 2026-08-08

### Summary

- **v0.9.14: Workflow Visibility & Usage Pace** — Adds clearer GitHub workflow controls, paced usage indicators, and smoother pane interactions.

### Added

- **Hidden workflow runs** — GitHub panes can now manage hidden workflow runs so noisy or irrelevant runs stay out of the way.
- **TwiCC usage pace bars** — TwiCC usage views now show paced bars that make current usage easier to scan.

### Changed

- **GitHub refresh pacing** — GitHub data refreshes are now scheduled more consistently to reduce bursts and keep the pane responsive.
- **Pane expansion tooltip** — Pane expansion controls now describe their behavior more clearly.

### Fixed

- **Split resizers** — Pane split resizers remain interactive after recent layout expansion changes.
...
- **v0.9.9: Reorderable Session Flow** — TwiCC Session Flow cards can now be manually rearranged so each lane matches your working order.

### Changed

- **Reorderable session cards** — TwiCC Session Flow cards can be moved within their lanes, making it easier to prioritize and organize active work.
...
- **v0.9.3: Smarter project panes** — Boatyard reuses existing web app panes, respects global Pier URLs, and keeps workspace state updates reliable.

### Changed

- **Web app pane reuse** — Opening a transient web app now reuses its existing pane instead of creating duplicates.
- **Global Pier URL** — Project panes now use the globally configured Pier URL when available.

### Fixed

- **Workspace state updates** — Workspace changes now return the correct updated state, preventing stale or inconsistent interface responses.

## [0.9.2] - 2026-07-22

### Summary

- **v0.9.2: Smoother project setup** — Boatyard improves project registration and restores reliable directory selection on KDE desktops.

### Added

- **TwiCC setup for restored projects** — Restored Boatyard projects can now be registered with TwiCC directly from project settings.

### Fixed

- **KDE directory picker** — Project directory selection now works correctly on KDE desktops.
...
- **v0.8.8: TwiCC menu stability** — The TwiCC top bar usage menu now opens more reliably without layout shifts.

### Fixed

- **Stable TwiCC usage menu** — Right-clicking the TwiCC top bar usage chip now positions its display menu within the window and keeps the surrounding top bar layout steady.
